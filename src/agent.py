...
# Inicializa o modelo de embeddings (Ollama local)
# Nota: llama3.1 e llama3.2 suportam function calling
...
def consultar_BD(pergunta: str) -> str:
    ...
    Returns:
        String com os 3 resumos mais similares encontrados (k=3)
    """
    try:
        ...
        if not documentos:
            return "Não encontrei informações sobre isso no banco de dados."
        ...
        # Formata resposta
        resultado = "🔍 Encontrei os 3 resumos mais similares no banco:\n\n"
        
        for idx, (doc, score) in enumerate(top_3, 1):
            resultado += f"{idx}. [Similaridade: {score:.3f}]\n"
...
1. Quando receber uma PERGUNTA:
   ...
   - Se encontrar informação relevante: Use-a como base e complemente se necessário
   - Se NÃO encontrar: Use 'buscar_referencias' para buscar na internet
   - Apresente a resposta ao usuário
   - Continue conversando e refinando conforme necessário
...
IMPORTANTE:
- consultar_BD usa EMBEDDINGS e retorna os 3 mais similares (não é busca exata)
- gerar_resumo analisa TODO o histórico da conversa atual
- O resumo é salvo automaticamente no banco após ser gerado
