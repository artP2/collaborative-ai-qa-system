...
with st.sidebar:
    ...
    tema_resumo = st.text_input("Defina o tema:", value=st.session_state.tema_value)
...
for msg in st.session_state.lc_history:
    if isinstance(msg, HumanMessage):
        ...
        # (Ignoramos mensagens de 'tool_calls' e mensagens intermediárias/erro)
        if msg.content and not any(phrase in msg.content.lower() for phrase in [
...
if user_input := st.chat_input("Faça sua pergunta..."):
    ...
    # Filtra mensagens de erro/intermediárias
    if resultado["response"] and not any(phrase in resultado["response"].lower() for phrase in [
        "desculpe pelo problema",
        "vou tentar novamente",
        "tentando novamente"
    ]):
